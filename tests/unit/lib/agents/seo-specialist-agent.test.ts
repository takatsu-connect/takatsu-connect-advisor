 ...
 *
 * seo-specialist エージェント定義の統合テスト
 *
 * テスト対象:
 *   - prompts/agents/seo-specialist.md の存在・内容検証
 *   - prompts/tools/query_search_console.md の存在確認
 *   - prompts/tools/fetch_webpage.md の存在確認
...
describe("seo-specialist ファイル存在確認", () => {
  test("1-1. prompts/agents/seo-specialist.md が存在する", async () => {
    // Act & Assert
    await expect(access(SEO_SPECIALIST_MD_PATH)).resolves.toBeUndefined();
  });
  ...
  test("1-3. prompts/tools/fetch_webpage.md が存在する", async () => {
    // Act & Assert
    await expect(access(FETCH_WEBPAGE_MD_PATH)).resolves.toBeUndefined();
  });

  test("1-4. prompts/tools/fetch_wp_posts.md が存在する", async () => {
    // Act & Assert
...
describe("loadAgentDefinition() - seo-specialist.md フィールド検証", () => {
  let definition: Awaited<ReturnType<typeof loadAgentDefinition>>;

  beforeAll(async () => {
    clearAgentCache();
    definition = await loadAgentDefinition(SEO_SPECIALIST_MD_PATH);
  });
  ...
  test("2-1. name === 'seo-specialist'", () => {
    // Assert
    expect(definition.name).toBe("seo-specialist");
  });

  test("2-2. displayName === 'SEO専門家'", () => {
    // Assert
    expect(definition.displayName).toBe("SEO専門家");
  });
  ...
  test("2-5. tools に 'query_search_console', 'fetch_webpage', 'fetch_wp_posts' がすべて含まれる", () => {
    // Assert
    expect(definition.tools).toContain("query_search_console");
    expect(definition.tools).toContain("fetch_webpage");
...
describe("loadAgentDefinition() - systemPrompt 内容確認", () => {
  ...
  beforeAll(async () => {
    clearAgentCache();
    const definition = await loadAgentDefinition(SEO_SPECIALIST_MD_PATH);
  ...
  test("4-2. systemPrompt に 'query_search_console' が含まれる（ツール使用ガイド）", () => {
    // Assert
