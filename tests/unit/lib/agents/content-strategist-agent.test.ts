 ...
 *
 * content-strategist エージェント定義の統合テスト
 *
 * テスト対象:
 *   - prompts/agents/content-strategist.md の存在・内容検証
...
const CONTENT_STRATEGIST_MD_PATH = path.join(
  PROJECT_ROOT,
  "prompts",
  "agents",
...
describe("content-strategist ファイル存在確認", () => {
  test("1-1. prompts/agents/content-strategist.md が存在する", async () => {
    // Act & Assert
    await expect(access(CONTENT_STRATEGIST_MD_PATH)).resolves.toBeUndefined();
  });

  test("1-2. prompts/tools/fetch_wp_posts.md が存在する", async () => {
    // Act & Assert
...
describe("loadAgentDefinition() - content-strategist.md フィールド検証", () => {
  let definition: Awaited<ReturnType<typeof loadAgentDefinition>>;

  beforeAll(async () => {
    clearAgentCache();
    definition = await loadAgentDefinition(CONTENT_STRATEGIST_MD_PATH);
  });
  ...
  test("2-1. name === 'content-strategist'", () => {
    // Assert
    expect(definition.name).toBe("content-strategist");
  });

  test("2-2. displayName === 'コンテンツ戦略家'", () => {
    // Assert
    expect(definition.displayName).toBe("コンテンツ戦略家");
  });
  ...
  test("2-5. tools に 'fetch_wp_posts' と 'query_google_analytics' がすべて含まれる", () => {
    // Assert
...
describe("loadAgentDefinition() - systemPrompt 内容確認", () => {
  ...
  beforeAll(async () => {
    clearAgentCache();
    const definition = await loadAgentDefinition(CONTENT_STRATEGIST_MD_PATH);
  ...
  test("4-1. systemPrompt に 'コンテンツ' または '記事' または '企画' が含まれる（役割説明）", () => {
    // Assert
    const hasRoleDescription =
      systemPrompt.includes("コンテンツ") ||
      systemPrompt.includes("記事") ||
      systemPrompt.includes("企画");
  ...
  test("4-3. systemPrompt に 'query_google_analytics' が含まれる（ツール使用ガイド）", () => {
    // Assert
