 ...
 *
 * marketing-specialist エージェント定義の統合テスト
 *
 * テスト対象:
 *   - prompts/agents/marketing-specialist.md の存在・内容検証
...
const MARKETING_SPECIALIST_MD_PATH = path.join(
  PROJECT_ROOT,
  "prompts",
  "agents",
...
describe("marketing-specialist ファイル存在確認", () => {
  test("1-1. prompts/agents/marketing-specialist.md が存在する", async () => {
    // Act & Assert
    await expect(access(MARKETING_SPECIALIST_MD_PATH)).resolves.toBeUndefined();
  });
...
describe("loadAgentDefinition() - marketing-specialist.md フィールド検証", () => {
  let definition: Awaited<ReturnType<typeof loadAgentDefinition>>;

  beforeAll(async () => {
    clearAgentCache();
    definition = await loadAgentDefinition(MARKETING_SPECIALIST_MD_PATH);
  });
  ...
  test("2-1. name === 'marketing-specialist'", () => {
    // Assert
    expect(definition.name).toBe("marketing-specialist");
  });

  test("2-2. displayName === 'マーケティング専門家'", () => {
    // Assert
    expect(definition.displayName).toBe("マーケティング専門家");
  });
...
describe("loadAgentDefinition() - systemPrompt 内容確認", () => {
  ...
  beforeAll(async () => {
    clearAgentCache();
    const definition = await loadAgentDefinition(MARKETING_SPECIALIST_MD_PATH);
  ...
  test("4-1. systemPrompt に 'マーケティング' または '集客' が含まれる（役割説明）", () => {
    // Assert
    const hasRoleDescription =
      systemPrompt.includes("マーケティング") || systemPrompt.includes("集客");
